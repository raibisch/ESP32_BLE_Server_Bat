...
#define AlertNotificationService BLEUUID((uint16_t)0x1811) 
BLECharacteristic AlertLevelCharacteristic(BLEUUID((uint16_t)0x2A06), BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY);
BLEDescriptor AlertLevelDescriptor(BLEUUID((uint16_t)0x2901));
...
void InitBLE() 
{
  BLEDevice::init("UNTERDORF");
  // Create the BLE Server
  BLEServer *pServer = BLEDevice::createServer();
  pServer->setCallbacks(new MyServerCallbacks());

  // Create the BLE BatteryService
  BLEService *pBattery = pServer->createService(BatteryService);
  pBattery->addCharacteristic(&BatteryLevelCharacteristic);
  BatteryLevelDescriptor.setValue("0-100 Prozent");
  BatteryLevelCharacteristic.addDescriptor(&BatteryLevelDescriptor);
  BatteryLevelCharacteristic.addDescriptor(new BLE2902());
  pServer->getAdvertising()->addServiceUUID(BatteryService);
  
  
  // Create the BLE AlertNotificationService
  BLEService *pAllert = pServer->createService(AlertNotificationService);
  pAllert->addCharacteristic(&AlertLevelCharacteristic);
  AlertLevelDescriptor.setValue("Aber mal HALLO!");
  AlertLevelCharacteristic.addDescriptor(&AlertLevelDescriptor);
  AlertLevelCharacteristic.addDescriptor(new BLE2902());
  pServer->getAdvertising()->addServiceUUID(AlertNotificationService);
  ...
  AlertLevelCharacteristic.setValue("");
}
...
void loop() 
{
  ...
  std::string sAlert = AlertLevelCharacteristic.getValue();
  String *str3 = new String(sAlert.c_str());
  
  String s4 = str3->c_str();
  Serial.print("Alert-Read:");
  Serial.printf("%0x",s4[0]);
  Serial.println(":");
  if (s4[0] == 0)
  {
